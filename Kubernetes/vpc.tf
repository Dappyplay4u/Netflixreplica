...
resource "aws_vpc" "netflix-eks-cluster-main" {
...
resource "aws_internet_gateway" "netflix-eks-cluster-igw" {
...
resource "aws_eip" "netflix-eks-cluster-nat" {
  vpc = true
...
resource "aws_nat_gateway" "netflix-eks-cluster-nat" {
  allocation_id = aws_eip.netflix-eks-cluster-nat.id
  ...
  depends_on = [aws_internet_gateway.netflix-eks-cluster-igw]
}
...
resource "aws_route_table" "private" {
  ...
  route {
    ...
    nat_gateway_id = aws_nat_gateway.netflix-eks-cluster-nat.id
  }
...
resource "aws_route_table" "public" {
  ...
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.netflix-eks-cluster-igw.id
  }
