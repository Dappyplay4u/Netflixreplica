...
resource "aws_security_group" "netflix-eks-cluster" {
  name        = "netflix-EKS-cluster-sg"
  description = "Cluster communication with worker nodes"
  vpc_id      = aws_vpc.netflix-eks-cluster-main.id
...
resource "aws_iam_role" "netflix-eks-cluster" {
...
resource "aws_iam_role_policy_attachment" "netflix-eks-cluster-policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
  role       = aws_iam_role.netflix-eks-cluster.name
}
...
resource "aws_eks_cluster" "netflix-eks-cluster" {
  name     = var.cluster_name
  version  = var.cluster_version
  role_arn = aws_iam_role.netflix-eks-cluster.arn

  vpc_config {
    security_group_ids      = [aws_security_group.netflix-eks-cluster.id]
  ...
  depends_on = [aws_iam_role_policy_attachment.netflix-eks-cluster-policy]
}
